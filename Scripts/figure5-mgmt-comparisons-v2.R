...
# CJ Brown 2024-02-19
# Figure 5 
#This is fininiky - getting the right mgmt comparisons takes lots of joins
...
light_mgmt <- left_join(baseline, light, by = c("phi", "n_sources", "n", "model")) %>%
  mutate(`Gain (years)` = (median_recovery.x - median_recovery.y)/365,
         ...
         per5 = per5.y, 
         per95 = per95.y) %>%
  select(model, Scenario, n_sources, I., phi, 
         `Gain (years)`, `Gain (times faster)`,
         median_recovery, 
         per5, per95)
...
#
# Recovery times under scnrs
#


g1 <- mgmt_scnrs %>%
  filter(.,phi<600, light_lev==0.5) %>%
  ggplot() +
...
#
#Above but as a lineplot of mgmt gain
#


g3 <- 
  filter(mgmt_scnrs, n_sources %in% c(1,8) & 
           phi<600, light_lev>0.3) %>%
  ggplot() +
  aes(x = light_lev, 
      y = `Gain (years)`,
      # y = `Gain (times faster)`, 
  ...
  xlab("Light level (proportion of normal)") +
...
ggsave(g3, file = "Outputs/fig5-mgmt-gain-zostera-curves.png",
       width = 6, height = 3.5)
